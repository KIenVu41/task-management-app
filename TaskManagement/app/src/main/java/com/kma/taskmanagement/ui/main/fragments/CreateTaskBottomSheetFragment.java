    ...
    private void createTask() {
       ...
       String minEnd = itemTime1[1];
        ...
        if(Utils.isNetworkConnected(requireActivity())) {
            if(isEdit) {
                cancleAlarm();
                taskViewModel.update(token, task.getId(), new Task("", cateId, "", desc, endDateFormat,null,  title, GlobalInfor.username, prio,  startDateFormat, status, null));
            } else {
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    taskViewModel.getTasksByCategory(token, cateId);
                }
            },500);
